...
const PILL_BRAND =
  "inline-flex w-full items-center justify-center rounded-full bg-[#2ED8A8] px-7 py-4 font-sans text-[12px] font-semibold tracking-[0.16em] uppercase text-[#141414] transition-colors hover:bg-[#1BAF86]";
const PILL_ACTION =
    ...
    >
      ...
      <section className="relative overflow-hidden bg-white text-[#141414]">
        <div className={`relative ${WRAP} pt-[clamp(72px,10vw,118px)] pb-[clamp(54px,7vw,82px)]`}>
          <p className="flex items-baseline gap-3.5 font-sans text-[12px] font-medium tracking-[0.22em] uppercase text-[#8E8E8E]">
            Zential Pure · Protocols · <b className="font-sans font-light text-[26px] tabular-nums tracking-normal text-[#0E7A54]">( 01 – 03 )</b>
          </p>
          <h1 className="my-[22px] max-w-[13ch] font-sans font-light tracking-[-0.03em] leading-[1.02] text-[#141414] text-[clamp(44px,6vw,88px)]">
            Three protocols.<br />One <span className="text-[#0E7A54]">system.</span>
          </h1>
      ...
      {/* ── PRINCIPLE ── */}
      <section className="border-y border-[rgba(20,20,20,0.10)] bg-white py-[clamp(72px,10vw,120px)] text-[#141414]">
        <div className={`${WRAP} mx-auto max-w-[880px] text-center`}>
          <p className="font-sans font-light text-[clamp(26px,3.4vw,44px)] leading-[1.18] tracking-[-0.025em] text-[#141414]">
            A device works because of its <span className="text-[#0E7A54]">mechanism</span>.<br />A protocol works because of its <span className="text-[#0E7A54]">order</span>.
          </p>
          <div className="mt-[26px] font-sans text-[10px] font-medium tracking-[0.3em] uppercase text-[#8E8E8E]">Discipline is just order, kept</div>
        </div>
      </section>

      {/* ── PROTOCOLS ── */}
      <section className="bg-white py-[clamp(72px,10vw,120px)] text-[#141414]">
        <div className={WRAP}>
          {PROTOCOLS.map((p, idx) => (
            <div key={p.num} className={idx > 0 ? "mt-[clamp(56px,8vw,96px)]" : ""}>
              <div className="grid grid-cols-1 items-end gap-6 border-b border-[rgba(20,20,20,0.10)] pb-[26px] md:grid-cols-[1fr_auto]">
                <div>
                  <div className="flex items-center gap-3 font-sans text-[11px] font-medium tracking-[0.22em] uppercase text-[#8E8E8E]">
                    <span className="text-[#0E7A54]">Protocol ( {p.num} )</span> · {p.kicker}
                  </div>
                  <h2 className="mt-3.5 font-sans font-light text-[clamp(34px,4.4vw,56px)] leading-none tracking-[-0.03em] text-[#141414]">{p.title}</h2>
                </div>
                <div className="md:text-right">
                  <div className="font-sans text-[11px] font-medium tracking-[0.16em] uppercase text-[#8E8E8E]">{p.seqLabel}</div>
                  <div className="mt-1 font-sans font-light tabular-nums text-[30px] leading-[1.1] text-[#141414]">{p.price} <small className="text-[14px] text-[#8E8E8E]">once</small></div>
                </div>
              </div>

              <div className="mt-[38px] grid grid-cols-1 items-start gap-[clamp(36px,5vw,72px)] md:grid-cols-[1.35fr_1fr]">
                <ol className="relative m-0 list-none p-0 before:absolute before:left-[19px] before:top-3.5 before:bottom-3.5 before:w-px before:bg-[rgba(20,20,20,0.10)]">
                  {p.steps.map((s) => (
                    <li key={s.n} className="relative grid grid-cols-[40px_1fr] gap-[22px] pb-7 last:pb-0">
                      <span className="relative z-[2] flex h-10 w-10 items-center justify-center rounded-full border border-[#0E7A54] bg-white font-sans text-[13px] font-medium tabular-nums text-[#0E7A54]">{s.n}</span>
                      <div className="pt-0.5">
                        <div className="mb-1.5 flex flex-wrap items-baseline gap-3.5">
                          <span className="font-sans text-[22px] tracking-[-0.02em] text-[#141414]">{s.verb}</span>
                          <span className="font-sans text-[10px] font-medium tracking-[0.16em] uppercase text-[#8E8E8E]">{s.mod}</span>
                          <span className="ml-auto font-sans text-[11px] tabular-nums text-[#0E7A54]">{s.min}</span>
                        </div>
                        <p className="m-0 max-w-[46ch] text-[14px] leading-[1.65] text-[#5A5A5A]">{s.body}</p>
                      </div>
                    </li>
                  ))}
                </ol>
                ...
                <aside className="md:sticky md:top-[88px]">
                  <div className="mb-5 aspect-[16/10] overflow-hidden rounded-none bg-[#F2F4F3] md:aspect-[4/5]">
                    <img src={p.img} alt={p.imgAlt} className="h-full w-full object-cover" />
                  </div>
                  <div className="rounded-none border border-[rgba(20,20,20,0.10)] bg-white p-[26px] shadow-[0_18px_50px_rgba(20,20,20,0.06)]">
                    {[["Instrument", p.instrument], ["Modalities", p.modalities], ["Cadence", p.cadence]].map(([k, v], i) => (
                      <div key={k} className={i > 0 ? "mt-4 border-t border-[rgba(20,20,20,0.10)] pt-4" : ""}>
                        <div className="font-sans text-[10px] font-medium tracking-[0.18em] uppercase text-[#8E8E8E]">{k}</div>
                        <div className="mt-0.5 text-[14px] text-[#141414]">{v}</div>
                      </div>
                    ))}
      ...
      <section className="relative flex min-h-[clamp(360px,46vw,520px)] items-center overflow-hidden bg-[#141414]">
        ...
        <div className={`relative z-[2] ${WRAP}`}>
          <p className="max-w-[16ch] font-sans font-light tracking-[-0.025em] text-[clamp(26px,3.6vw,46px)] leading-[1.16] text-white">The sum of its parts, kept in order.</p>
          <div className="mt-[22px] font-sans text-[10px] tracking-[0.3em] uppercase text-white/70">Three vessels · three sequences · order kept</div>
        </div>
      </section>
      ...
      <section className="border-t border-[rgba(20,20,20,0.10)] bg-white py-[clamp(72px,10vw,120px)] text-[#141414]">
        <div className={`${WRAP} mx-auto max-w-[760px] text-center`}>
          <p className="inline-flex items-center justify-center gap-3.5 font-sans text-[11px] font-medium tracking-[0.22em] uppercase text-[#8E8E8E]">
          ...
          <h2 className="my-5 font-sans font-light tracking-[-0.025em] text-[clamp(28px,3.4vw,44px)] text-[#141414]">Start with one sequence. The system follows.</h2>
          <p className="mx-auto mb-[34px] max-w-[540px] text-[17px] leading-[1.75] text-[#5A5A5A]">
          ...
          <div className="flex flex-wrap justify-center gap-3.5">
            ...
            <Link to="/instruments" className={PILL_GHOST}>See all instruments</Link>
          </div>
        </div>
      </section>
    </PageShell>

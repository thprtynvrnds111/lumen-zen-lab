...
/**
 * "The Recovery System" — the two body instruments (Restoration Belt + Restoration
 * Mat) in one purchase. Backed by a real Shopify bundle product
 * (SKU ZP-RECOVERY-BUNDLE, €420) sold through a Shopify cart permalink, so checkout
 * works end-to-end without a discount-code scope.
 *
 * Fulfilment note: an order for this bundle is one line item — ship both
 * instruments (Restoration Belt + Restoration Mat) and adjust component inventory
 * manually. To change the price, edit the bundle product in Shopify admin and
 * update BUNDLE_PRICE here.
 */
...
const FULL_PRICE = SYSTEM.reduce((s, i) => s + i.price, 0); // €500
const BUNDLE_PRICE = 420;

// Live Shopify bundle variant (product: the-recovery-system).
const BUNDLE_VARIANT_ID = "53899002642775";
...
export function RecoverySystem() {
  ...
  function claimRecovery() {
    ...
    if (w.fbq) w.fbq("track", "AddToCart", { content_name: "The Recovery System", value: BUNDLE_PRICE, currency: "EUR" });
    if (w.gtag) w.gtag("event", "add_to_cart", { item_name: "The Recovery System", value: BUNDLE_PRICE, currency: "EUR" });
    window.location.href = safeCheckoutUrl(BUNDLE_PERMALINK);
  }

  return (
    <section className="bg-[#1A1714] pb-[clamp(72px,10vw,116px)] text-[#F7F4F0]">
      <div className={WRAP}>
        <div className="grid items-center gap-10 rounded-[16px] border border-[rgba(247,244,240,0.10)] bg-[#070A0E] p-[clamp(28px,4vw,56px)] md:grid-cols-[1.05fr_0.95fr]">
          <div>
            <div className="font-sans text-[11px] tracking-[0.3em] uppercase text-[#2ED8A8]">The Recovery System</div>
            <h2 className="mt-4 font-serif italic font-normal text-[clamp(28px,3.6vw,44px)] leading-[1.08] text-[#F7F4F0]">
              Targeted and full-body. The recovery layer.
            </h2>
            <p className="mt-4 max-w-[46ch] text-[15px] leading-[1.7] text-[#F7F4F0]/[0.62]">
              The Belt presses red light into the muscle you worked. The Mat lays the whole body down at night. Two instruments, one recovery ritual.
            </p>
          ...
          <div className="rounded-[14px] border border-[rgba(247,244,240,0.10)] bg-[#1A1714] p-[clamp(26px,3vw,40px)]">
            ...
            <div className="mt-1 font-serif italic text-[18px] text-[#C6A07C]">Recovery System · save €{FULL_PRICE - (BUNDLE_PRICE as number)}</div>
            <button
            ...
            >
              {busy ? "Adding the Recovery System…" : "Claim the Recovery System"}
            </button>
            <p className="mt-3 text-center font-sans text-[11px] text-[#F7F4F0]/45">30-day protocol guarantee on both instruments.</p>
          </div>
        </div>
      </div>
    </section>
